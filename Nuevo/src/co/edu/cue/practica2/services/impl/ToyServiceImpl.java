...
public class ToyServiceImpl implements Toy {
    ...
    public Juguete informeFiltroUsuario(double precioUsuario) {
        ...
        System.out.println("Los juguetes con un valor mayor a: "+precioUsuario+" son: \n");
        for (Juguete juguete : toys) {
            if (String.valueOf(juguete) != "null") {
                JOptionPane.showMessageDialog(null,juguete.getNombre());
                if(precioUsuario<=juguete.getPrecio()){
                    System.out.println(juguete.getNombre() +" y su precio es: "+juguete.getPrecio());
                    existencia = false;
                }
            }
        }
        if(existencia==true){
            JOptionPane.showMessageDialog(null, "No hay ningun juguete mayor a este precio");
        }
        return null;
    }
